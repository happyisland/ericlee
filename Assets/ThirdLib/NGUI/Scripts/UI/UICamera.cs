	...
	static public void Notify (GameObject go, string funcName, object obj)
	{
		...
		if (NGUITools.GetActive(go))
		{
            ...
            if (genericEventHandler != null)
            {
                if (go != null)
                {
                    genericEventHandler.SendMessage("GuideEvent", new EventReceive(funcName, go, obj), SendMessageOptions.DontRequireReceiver);
                }
            }
            else
            {
                ...
                if (GuideViewBase.Ins != null && GuideViewBase.Ins.isTriggle)
                {
                    if (triggleObj == null)
                    {
                        triggleObj = GameObject.Find(StepManager.GetIns().GetCurStepParth()); //找到nullstep的触发目标
                    }
                    if (funcName.Contains("OnClick") && triggleObj != null && go == triggleObj)
                    {
                        GuideViewBase.Ins.EndTriggleStep();
                        triggleObj = null;
                        genericEventHandler.SendMessage("GuideEvent", new EventReceive(funcName, go, obj), SendMessageOptions.DontRequireReceiver);
                        GuideViewBase.Ins.ShowAfterTriggle();
                        mNotifying = false;
                        return;
                    }
                }
