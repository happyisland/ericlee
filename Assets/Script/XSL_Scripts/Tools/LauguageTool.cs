...
public class LauguageTool
{
    ...
    { 
        try
        {
            if (!System.IO.File.Exists(LauguageConfigPath)) //  读取文件 文件不存在默认中文
            {
                //CurLauguage = LauguageType.Chinese;
                //CurLauguage = LauguageType.Arab;
                SetLauguage(LauguageType.English);
                //   CurLauguage = LauguageType.Italy;
            }
            else  // 1为真
            {
                string msg = System.IO.File.OpenText(LauguageConfigPath).ReadToEnd();
                Dictionary<string, object> data = (Dictionary<string, object>)LitJson.Json.Deserialize(msg);
                if (data != null)
                {
                    string inst = data["LanguageCode"].ToString();
                    if (inst == "China")
                    {
                        CurLauguage = LauguageType.Chinese;
                        return;
                    }
                    else if (inst == "English")
                    {
                        CurLauguage = LauguageType.English;
                    }
                    else if (inst == "German")
                    {
                        CurLauguage = LauguageType.German;
                    }
                    else if (inst == "Italy")
                    {
                        CurLauguage = LauguageType.Italy;
                    ...
                    else if (inst == "Korean")  //韩语目前没翻译 默认英文
                    {
                        CurLauguage = LauguageType.Korean;
                    }
                    else if (inst == "French") //法语
                    {
                        CurLauguage = LauguageType.French;
                    }
                    else if (inst == "Spanish") //西班牙语
                    {
                        CurLauguage = LauguageType.Spanish;
                    }
                    else if (inst == "Portugal")  //葡萄牙语
                    {
                        CurLauguage = LauguageType.Portugal;
                    }
                    else if (inst == "HokongChinese")  //繁体中文
                    {
                        CurLauguage = LauguageType.HokongChinese;
                    }
                    else if (inst == "Arab")   //阿拉伯  暂无翻译，英文替代
                    {
                        CurLauguage = LauguageType.Arab;  
                    }
