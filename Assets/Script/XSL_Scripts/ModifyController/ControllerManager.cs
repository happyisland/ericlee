...
/// <summary>
/// author:   孙宇
/// describe: 遥控器数据管理  controllerData负责控件的数据 widgetManager负责控件的逻辑 
/// </summary>
public class ControllerManager : SingletonObject<ControllerManager>
{
    //private string modeType = "/default/";
    private static string ControllerPath = Application.persistentDataPath + "/default/" + RobotManager.GetInst().GetCurrentRobot().Name.Substring(0, RobotManager.GetInst().GetCurrentRobot().Name.IndexOf("_")) + "/ControllerData"; //遥控器数据存放路径
    private string ControllerID; //遥控器名称
    private ControllerData preControllerData;  //配置前的遥控器数据
    private ControllerData curController; //当前遥控器数据
    public string ControllerName;  //遥控器名字
