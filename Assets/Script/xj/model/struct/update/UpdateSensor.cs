...
public class UpdateSensor : UpdateBase
{
    ...
    #region 其他属性
    protected TopologyPartType mSensorType;
    #endregion
    ...
    public override ErrorCode CheckUpdate(ReadMotherboardDataMsgAck msg)
    {
        ...
        do
        {
            SensorData sensorData = msg.GetSensorData(mSensorType);
            if (null != sensorData)
            {
                PlatformMgr.Instance.Log(MyLogType.LogTypeDebug, string.Format("CheckUpdate SensorType = {0} mVersion = {1} mFilePath = {2}", mSensorType.ToString(), mVersion, mFilePath));
                if (sensorData.ids.Count > 0 && sensorData.errorIds.Count < 1 && !string.IsNullOrEmpty(mVersion) && (!mVersion.Equals(sensorData.version) || sensorData.errorVerIds.Count > 0))
                {
                    ...
                    {
                        if (!mFilePath.EndsWith(".bin") || File.Exists(mFilePath))
                        {
                ...
                {
                    PlatformMgr.Instance.Log(MyLogType.LogTypeDebug, mSensorType.ToString() + "不需要升级");
                }
            }
        } while (false);
        return ret;
    ...
    public override bool UpdateStart(Robot robot, byte arg)
    {
        ...
        if (flag)
        {
            ...
            msg.sensorType = (byte)mSensorType;
            msg.ids=arg;
