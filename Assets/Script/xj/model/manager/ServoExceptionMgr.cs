...
public class ServoExceptionMgr : SingletonObject<ServoExceptionMgr>
{
    ...
    public List<byte> GetSensorException(ServoExceptionType exception)
    {
        if (exception == mExceptionType)
        {
    ...
    {
        mRobot = robot;
        if (SingletonObject<ServoExceptionMgr>.GetInst().GetServoException() != ServoExceptionType.None)
        {
            if (SingletonObject<ServoExceptionMgr>.GetInst().GetServoException() == ServoExceptionType.otherProtect || SingletonObject<ServoExceptionMgr>.GetInst().GetServoException() == ServoExceptionType.encryptProtect)
            {
            ...
            else
            {
                if (LogicCtrl.GetInst().IsLogicProgramming)
                {
                    LogicCtrl.GetInst().ExceptionCallBack(SingletonObject<ServoExceptionMgr>.GetInst().GetExceptionTips(), SelfServoExceptionOnClick);
                }
                else
                {
                    PromptMsg.ShowDoublePrompt(SingletonObject<ServoExceptionMgr>.GetInst().GetExceptionTips(), SelfServoCheckErrorOnClick);
                }
            }
            return true;
        }
