...
public class PromptDevice : BasePopWin
{
    ...
    public static void ShowMainboardInfo(ReadMotherboardDataMsgAck boardData)
    {
        ...
        args[0] = TopologyPartType.MainBoard;
        PromptDevice msg = (PromptDevice)SingletonObject<PopWinManager>.GetInst().ShowPopWin(typeof(PromptDevice), args);
        ...
        right.Add(SingletonObject<ConnectManager>.GetInst().GetConnectedName());
    ...
    public static void ShowSensorInfo(TopologyPartType sensorType, int id, string version)
    {
        ...
        switch (sensorType)
        {
            ...
            case TopologyPartType.Gyro:
                msg.SetMainTitle(LauguageTool.GetIns().GetText("陀螺仪传感器"));
                break;
        ...
        try
        {
            if (null != mTrans)
            {
                ...
                if (null != contents)
                {
                    ...
                    if (TopologyPartType.MainBoard != mDeviceType)
                    {
