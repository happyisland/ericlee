...
public class SetDeviceUI : BaseUI
{
        ...
        try
        {
            ...
            if (null != mTrans)
            {
                ...
                if (null != title)
                {
                    ...
                    if (null != lb)
                    {
                        lb.text = LauguageTool.GetIns().GetText("修改舵机ID");
                    }
                }
                ...
                Transform connectBtn = mTrans.Find("list/connectBtn");
                if (null != connectBtn)
                {
                    //connectBtn.localPosition = UIManager.GetWinPos(connectBtn, UIWidget.Pivot.BottomLeft, PublicFunction.Back_Btn_Pos.x, 110);
                    TweenPosition tween = connectBtn.GetComponent<TweenPosition>();
                    if (null != tween)
                    {
                        Vector3 pos = UIManager.GetWinPos(connectBtn, UIWidget.Pivot.TopRight, PublicFunction.Back_Btn_Pos.x, PublicFunction.Back_Btn_Pos.y);
                        connectBtn.localPosition = pos + new Vector3(300, 0);
                        GameHelper.PlayTweenPosition(tween, pos, 0.6f);
                    }
                    else
                    {
                        connectBtn.localPosition = UIManager.GetWinPos(connectBtn, UIWidget.Pivot.TopRight, PublicFunction.Back_Btn_Pos.x, PublicFunction.Back_Btn_Pos.y);
                    }
                    mConnectBtn = connectBtn.GetComponent<UIButton>();
                    mConnectIcon = GameHelper.FindChildComponent<UISprite>(connectBtn, "icon");
                }
                ...
                if (null != tips)
                {
                    ...
                    if (null != Label)
                    {
                        ...
                        if (null != lb)
                        {
                            lb.width = PublicFunction.GetWidth() - 320;
                            width = lb.width;
                            lb.text = LauguageTool.GetIns().GetText("修改舵机ID提示语");
                        }
                        Label.localPosition = new Vector3(-width / 2, (0.5f - 0.307f) * PublicFunction.GetHeight());
                    }
    ...
    void SetItem(int index, int id, GameObject obj)
    {
        ...
        {
            UILabel idLabel = GameHelper.FindChildComponent<UILabel>(obj.transform, "duoji/id");
            if (null != idLabel)
            {
        ...
        else
        {
            ...
            if (null != sensor)
            {
                ...
                UILabel label = GameHelper.FindChildComponent<UILabel>(sensor, "Label");
                if (null != label)
                {
    ...
    void SetInputColor(UIInput input, Color color)
    {
        ...
        UILabel lb = GameHelper.FindChildCompoent(input.transform, "Label", "UILabel") as UILabel;
        if (null != lb)
        {
            lb.color = color;
        }
    ...
    void ChangeCallBack(EventArg arg)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                HUDTextTips.ShowTextTip(LauguageTool.GetIns().GetText("SheBeiIdXiuGaiShiBai"));
            }
            CheckBtnState();
        }
        catch (System.Exception ex)
