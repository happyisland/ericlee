...
public class TopologyUI : BaseUI
{
    ...
    Vector2 mAngleSize = new Vector2(13, 13);
    ...
    string GetMissTips(TopologyPartType partType, string ids)
    {
        switch (partType)
        {
            case TopologyPartType.Servo:
                return string.Format(LauguageTool.GetIns().GetText("舵机ID"), ids);
            case TopologyPartType.Infrared:
                return string.Format(LauguageTool.GetIns().GetText("红外传感器ID"), ids);
    ...
    public void RecoverTopology()
    {
        ...
        foreach (KeyValuePair<GameObject, TopologyPartData> kvp in mTopologyDict)
        {
            if (kvp.Value.partType != TopologyPartType.MainBoard)
            {
                GameObject.Destroy(kvp.Key);
            }
            else
            ...
            if (null != center)
            {
                ...
                if (null != topologyPanel)
                {
                    ...
                    if (null != mPanelSelfTrans)
                    {
                        ...
                        mMotherBoxTrans = mPanelSelfTrans.Find("MotherBox");
                        if (null != mMotherBoxTrans)
                        {
                            ...
                            motherData.partType = TopologyPartType.MainBoard;
    ...
    GameObject CreateChoiceServo(TopologyPartData data, Transform parentTrans)
    {
        if (null != mServoItem)
        {
            ...
            if (null != lb)
            {
                lb.text = string.Format("ID-{0}", data.id);
            }
    ...
    GameObject CreateChoiceSensor(TopologyPartData data, Transform parentTrans)
    {
        if (null != mSensorItem)
        {
            ...
            SetSensorIcon(obj, data.partType);
    ...
    public static void SetSensorIcon(GameObject obj, TopologyPartType partType)
    {
        ...
        if (null != icon)
        {
            ...
            {
                case TopologyPartType.Infrared:
                    icon.spriteName = "infrared_icon";
                    break;
                case TopologyPartType.Gyro:
                    icon.spriteName = "Gyro_icon";
                    break;
                case TopologyPartType.Touch:
                    icon.spriteName = "Touch_icon";
                    break;
                case TopologyPartType.Light:
                    icon.spriteName = "Light_icon";
                    break;
                case TopologyPartType.DigitalTube:
                    icon.spriteName = "DigitalTube_icon";
                    break;
    ...
    void SetState(Transform trans, bool state)
    {
            ...
            case TopologyPartType.MainBoard:
                ...
                else
                {
                    if (mPortConnectionDict.ContainsKey(port))
                    {
                        ...
                        if (portName.StartsWith("s_") || portName.StartsWith("blank_"))
                        {
                            if (connected)
                            {
                                spriteName = "dian_liang";
        ...
        {//生成空白格与此端口连接
            ...
            if (null != obj)
            {
                ...
                if (mPartPortDict.ContainsKey(obj))
                {
                    ...
                    for (int i = 0, imax = list.Count; i < imax; ++i)
                    {
                        if (portId <= 3 && list[i].portObj.name.Equals("blank_left"))
                        {
                            conPort = list[i].portObj;
                            break;
                        }
                        else if (portId > 3 && list[i].portObj.name.Equals("blank_right"))
                        {
                            conPort = list[i].portObj;
                            break;
                        }
    ...
    void InitChoicePartQueue()
    {
        ...
        foreach (KeyValuePair<GameObject, TopologyPartData> kvp in mTopologyDict)
        {
            if (kvp.Value.partType == TopologyPartType.MainBoard || kvp.Value.partType == TopologyPartType.None)
            {
                continue;
            }
            if (kvp.Value.partType == TopologyPartType.Servo)
        ...
        if (null != servoList || null != sensorList)
        {
