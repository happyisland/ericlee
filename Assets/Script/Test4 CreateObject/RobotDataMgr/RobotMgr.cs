...
public class RobotMgr
{
   ...
   public bool leave=true;
    ...
    //创建GO
    public void CreateGO(string robotname,string goname,string goid,string gotype,int djid)
    {
    ...
    public Dictionary<string, Color> FindAllGOColor(string robotname)
    {
        ...
        for (int i = 1; i <= rbt[robotname].gos.Count; i++)      //i表示GO  id
        {
            foreach (string nam in rbt[robotname].gos.Keys)
            {
                ...
                Color tempcolor = StringToColor(gotypeTemp);
    ...
    public void ReviseDJID(string robotname, int oriid, int newid)
    {
        ...
        if (djname != null && djname != "")
        {
            if (hasNewDJid(robotname, newid) == false)
            {
    ...
    //是否包含新djid
    public bool hasNewDJid(string robotname, int newdjid)
    {
        bool hasnewid = false;
        if (RobotMgr.Instance.rbt.ContainsKey(robotname) == false)
        {
        ...
        if (RobotMgr.Instance.rbt[robotname].gos != null && RobotMgr.Instance.rbt[robotname].gos.Count > 0)
        {
            ...
            {
                string gotype = RobotMgr.Instance.rbt[robotname].gos[gonamTemp].goType;
                if ((gotype == "seivo" || gotype == "duoji") && RobotMgr.Instance.rbt[robotname].gos[gonamTemp].djID == newdjid)
                {
                    hasnewid = true;
                }
            }
    ...
    public static void SettingMatColor(GameObject go, Material mat,Color ori)
    {
        if (go != null)
        {
            ...
            if (grouptemp != null)
            {
                ...
                {
                    if (null != childL.transform.renderer && null != childL.transform.renderer.material)
                    {
                        ...
                        childL.transform.renderer.material = mat;
                        childL.transform.renderer.material.color = ori;
                        childL.transform.renderer.material.SetColor("_SpecColor", ori);
                    }
                }
            ...
            {
                // Debug.Log("origo:" + ori);
                if (null != go.transform.renderer && null != go.transform.renderer.material)
                {
                    ...
                    go.transform.renderer.material = mat;
                    go.transform.renderer.material.color = ori;
                    go.transform.renderer.material.SetColor("_SpecColor", ori);
                }
            }
        }
    }
    ...
    public static void SettingLineColor(GameObject go)
    {
        if (go != null)
        {
                ...
                if (nameT.Contains("line")||nameT.Contains("lnA")||nameT.Contains("lnB"))
                {
                    foreach (Transform childL in go.transform.GetComponentsInChildren<Transform>())
                    {
                        if (null != childL.transform.renderer && null != childL.transform.renderer.material)
                        {
                            ...
                            {
                                childL.transform.renderer.material.color = Color.white;
                                childL.transform.renderer.material.SetColor("_SpecColor", Color.white);
                            }
                            else if (nameChild.Contains("Line"))
                            {
                                childL.transform.renderer.material.color = Color.black;
                                childL.transform.renderer.material.SetColor("_SpecColor", Color.black);
                            }
                        }
                ...
                else
                {
                    foreach (Transform childL in go.transform.GetComponentsInChildren<Transform>())
                    {
                        if (null != childL.transform.renderer && null != childL.transform.renderer.material)
                        {
                            ...
                            if (nameChild.Contains("Object"))
                            {
                                childL.transform.renderer.material.color = OriColor;
                                childL.transform.renderer.material.SetColor("_SpecColor", OriColor);
                            }
                            else if (nameChild.Contains("Line"))
                            {
                                childL.transform.renderer.material.color = Color.black;
            ...
            else
            {
                ...
                if (null != go.transform.renderer && null != go.transform.renderer.material)
                {
                    go.transform.renderer.material.color = OriColor;
                    go.transform.renderer.material.SetColor("_SpecColor", OriColor);
                }
            }
        }
    }
        ...
        if (go != null)
        {
            if(go.name !="DP")   //go不为舵盘
            {
                foreach (Transform goDiffuse in go.transform.GetComponentInChildren<Transform>())
                {
                    ...
                    {
                        goDiffuse.renderer.material.shader = Shader.Find("Outlined/Silhouetted Bumped Diffuse");
                        goDiffuse.transform.renderer.material.color = ori;
                    }
                    else if (goDiffuse.name == "DP")
                    {
                        foreach (Transform goDif in goDiffuse.transform.GetComponentInChildren<Transform>())
                        {
                            goDif.renderer.material.shader = Shader.Find("Outlined/Silhouetted Bumped Diffuse");
                            goDif.transform.renderer.material.color = ori;
                        }
                    }
                    else if (goDiffuse.name == "Group")
                    {
                        foreach (Transform goDiffuseChild in goDiffuse.transform.GetComponentInChildren<Transform>())
                        {
                            ...
                            {
                                goDiffuseChild.renderer.material.shader = Shader.Find("Outlined/Silhouetted Bumped Diffuse");
                                goDiffuseChild.transform.renderer.material.color = ori;
                            }
                            else
                            {
            ...
            else
            {
                foreach (Transform goDiffuse in go.transform.GetComponentInChildren<Transform>())
                {
                    goDiffuse.renderer.material.shader = Shader.Find("Specular");
                    goDiffuse.transform.renderer.material.color = ori;
                }
            }
        }
    ...
    //将字符串转换为Vector3
    public static Vector3 StringToVector(string vect)
    {
    ...
    //将字符串转换为Vector3
    public static Color StringToColor(string vect)
    {
        string[] num = vect.Split(new char[] { '(', ',', ')' });
    ...
    //获得转换后的颜色
    public static Color GetColor(Color tempCol)
    {
        Color colorL = tempCol / 255;
        return colorL;
    }
    #endregion
