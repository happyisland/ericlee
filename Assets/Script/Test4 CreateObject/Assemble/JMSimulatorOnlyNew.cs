...
public class JMSimulatorOnlyNew : MonoBehaviour
{
    #region static
    public static JMSimulatorOnlyNew Instance;
    #endregion
   ...
   // private List<string> animtype = new List<string>();  //动画的source
    ...
    void Start()
    {
        ...
        #region 零件缺少提示
        string userID =PlatformMgr.Instance.GetUserData();
    ...
    public void FindComponents()
    {
        ...
        foreach (Transform child in camera.transform.GetComponentsInChildren<Transform>())
        {
            ...
            else if (child.name == "TestLabel" || child.name == "finishNotice2" || child.name == "finishNotice")
            {
       ...
       fsNotice = FindChildGO(uibottom, "fsNotice");
        ...
        inputlabels["fsStay"].text = nots["not7"];
        ...
        labels["finishNotice"].text = nots["not6"];
        labels["finishNotice2"].text = nots["not4"];
        FindLabel(startBtn).text = nots["not12"];

        FindChildGO(uicenter, "LackParts/lpnotice1").GetComponent<UILabel>().text = nots["not19"];
        FindChildGO(uicenter, "LackParts/lpnotice2").GetComponent<UILabel>().text = nots["not20"];
    ...
    public void FindOriClip()
    {
        ...
        if (RobotMgr.Instance.rbtAnim!=null)
        {
             rbtAnim=RobotMgr.Instance.rbtAnim;

             clipCount = rbtAnim[robotIDTemp].anims.Count;
        }
       
        if(RobotMgr.Instance.jubuPics !=null) { jubuPics = RobotMgr.Instance.jubuPics; }
        if (RobotMgr.Instance.anims != null) { anims = RobotMgr.Instance.anims; }
        if (RobotMgr.Instance.outPics != null) { outPics = RobotMgr.Instance.outPics; }
   ...
   #region 创建零件菜单列表
    ...
    public void AnimBaseHandle(int stepTempT)
    {
        ...
        if (stepTempT != 0)  
        {
            ...
            if (source == "djid" || source == "shape" || source == "line" || source == "pa")
            {
                DisableBtnClick(btns["Forward"]);
                DisableBtnClick(btns["Back"]);
                DisableBtnClick(btns["Return"]);
    ...
    public void JustButtonsEnabled(int stepTempT)
    {
        ...
        else if (stepTempT >= clipCount)   //搭建完成
        {
            ...
            inputlabels["fsName"].text = string.Format(nots["not5"], nameNoTypeT);//finishNotice
