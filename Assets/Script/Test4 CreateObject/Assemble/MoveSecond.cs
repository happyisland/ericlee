...
public class MoveSecond : MonoBehaviour
{
   ...
   public static Material matT;//默认使用的材质
    ...
    void Awake()
    {
       ...
       // Debug.Log("matT:"+matT.name);
        ...
        else if ("default" == RecordContactInfo.Instance.openType)
        {
           ...
           {
                mainUIRoot =GameObject.Find("MainUIRoot_new");
              ...
              loadingSprite.transform.parent = mainUIRoot.transform;
            }
             ...
             {
                 //Debug.Log("222111DSFSF");
                 //GameObject maingo = new GameObject();

                 //maingo.name = "animAssetsGO";
                 //if(maingo.GetComponent<AnimAssets>()==null)
                 //{
                 //    maingo.AddComponent("AnimAssets");
                 //    maingo.GetComponent<AnimAssets>().FindOriClip();
                 //}
                 Init();
             }
            else
    ...
    {
        if (SceneMgr.GetCurrentSceneType() == SceneType.MainWindow && "default" == RecordContactInfo.Instance.openType)
        {

            connect = GameObject.Find("MainUIRoot/ModelDetails/BottomLeft/grid_Officail/connect");
            Guanfang = GameObject.Find("MainUIRoot/ModelDetails/Right/grid_officail");
