...
public class PartsSprites : MonoBehaviour
{
    ...
    IEnumerator EnableSprite(float t,GameObject p)
    {
        ...
        if (p.GetComponent<UIGrid>()!=null)
        {
            ...
            p.GetComponent<UIGrid>().cellHeight = newP1Size.x;
            p.GetComponent<UIGrid>().cellWidth = newP1Size.y;
            p.GetComponent<UIGrid>().enabled = false;   
        }
        ...
        for(int i=0;i<sptPName.Count;i++)
        {
           if(i!=0)
           {
               ...
               panelT.transform.localPosition = new Vector3(posT.x+newP1Size.x*i,posT.y,posT.z);
    ...
    {
        #region 加载Clean图片
        //string pathTempT = "file:///" + Application.persistentDataPath + "//partsPic//Clean.png";

        //WWW wwwT = new WWW(pathTempT);


        //yield return wwwT;
        //if (wwwT != null && string.IsNullOrEmpty(wwwT.error))
        //{
        //   textureTT = wwwT.texture;
        //}
    ...
    public void CreateSprite(GameObject p,int m)
    {
        ...
        for (int t = 0; t < goCount;t++ )
        {
            ...
            if (nameTemp.Contains("label") == false)   //生成贴图
            {
                ...
                {
                    //Debug.Log("nameTemp  00:" + nameTemp);
