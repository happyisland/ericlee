...
- (void) ConnectDevice:(NSString *) mac
{
    ...
    {
        NSLog(@"ConnectDevice mac = %@ not found", mac);
...
- (void) DisConnectDevice
{
    ...
    for (uint8_t i = 0; i < [_connectedPeripheralList count]; i++) {
        ...
        {
            NSLog(@"DisConnectDevice mac = %@ not found", myPeripheral.advName);
        }
    }
}
...
- (void) centralManager:(CBCentralManager *)central didDiscoverPeripheral:(CBPeripheral *)aPeripheral advertisementData:(NSDictionary *)advertisementData RSSI:(NSNumber *)RSSI 
{
    ...
    NSLog(@"<---------\n[CBController] didDiscoverPeripheral, %@, count=%lu, RSSI=%d, count=%ld", aPeripheral.identifier, [advertisementData count], [RSSI intValue], [devicesList count]);
    
#else
    mac = aPeripheral.UUID.UUIDString;
    NSLog(@"<---------\n[CBController] didDiscoverPeripheral, %@, count=%d, RSSI=%d, count=%d", aPeripheral.UUID, [advertisementData count], [RSSI intValue], [devicesList count]);
...
- (void) centralManager:(CBCentralManager *)central didConnectPeripheral:(CBPeripheral *)aPeripheral 
{
    ...
    NSLog(@"[CBController] didConnectPeripheral, uuid=%@", aPeripheral.identifier);
...
- (void)centralManager:(CBCentralManager *)central didDisconnectPeripheral:(CBPeripheral *)aPeripheral error:(NSError *)error
{
    ...
    NSLog(@"[CBController] didDisonnectPeripheral uuid = %@, error msg:%ld, %@, %@", aPeripheral.identifier, error.code ,[error localizedFailureReason], [error localizedDescription]);
#else
    
    NSLog(@"[CBController] didDisonnectPeripheral uuid = %@, error msg:%ld, %@, %@", aPeripheral.UUID, error.code ,[error localizedFailureReason], [error localizedDescription]);
